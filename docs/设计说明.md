...
7. **skipped**（Dirty 有 `new`/`update` 但主缓存已无行）：直接丢弃该 Dirty 项。缓存行已不存在，恢复脏状态只会产生孤儿脏 key，没有意义。
8. flush 加锁后若处理抛错（非 DB 失败）：不再恢复 Dirty 项，flush 返回 `{error, Reason}` 并记日志。调用方需要处理失败；脏 key 已从 Dirty ETS 删除，不会自动重试。
...
| 路径 | whole / hotData 行为 |
|------|---------------------|
| `delete/2` | 删主缓存 + Dirty 标 `del`，flush 时删库 |
| `txn` 中 `del` | 同上，且提交失败可回滚 |

业务若需要删除参与事务，用 `txn` + `del`，不要依赖 `delete/2` 与 txn 混用。
...
- 单行加载失败（如 `tryLock` 冲突）立即中断加载并返回 `{error, {LockKey, Reason}}`，`csTabSrv` 初始化失败，避免「部分加载却看似成功」。
